...
export default function AuthSettings() {
  ...
  const fetch_platform_config = async () => {
    const response = await get(
      "/authentication_settings/list_platform_config/"
    );
    console.log("response :: ", response.data);
    if (response && response.data) {
      const keapConfig = response.data.find(
        (config) => config.platform_name === "keap"
      );
      const highLevelConfig = response.data.find(
        (config) => config.platform_name === "go_high_level"
      );
